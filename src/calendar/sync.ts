  ...
  private async _sync(): Promise<void> {
    ...
    try {
      ...
      for (const item of items) {
        ...
        const evData: Parameters<typeof upsertCalendarEvent>[0] = {
          google_id: item.id,
      ...
      deleteCalendarEventsNotIn(googleIds);
      process.stderr.write(`[calendar] synced ${googleIds.length} events\n`);
    } catch (e) {
      process.stderr.write(`[calendar] sync error: ${e instanceof Error ? e.message : String(e)}\n`);
    }
  }
}
