...
describe('bashTool', () => {
  ...
  it('executes a command and returns stdout', async () => {
    mockExeca.mockResolvedValueOnce({ all: 'hello', stdout: 'hello', stderr: '', exitCode: 0 } as any);
  ...
  it('includes stderr on non-zero exit', async () => {
    mockExeca.mockResolvedValueOnce({ all: 'some output', stdout: '', stderr: 'error msg', exitCode: 1 } as any);
  ...
  it('respects custom timeout within max ceiling', async () => {
    mockExeca.mockResolvedValueOnce({ all: '', stdout: '', stderr: '', exitCode: 0 } as any);
