...
export function fromOpenAIResponse(data: unknown, model: string): Anthropic.Message {
  ...
  if (choice.message.tool_calls) {
    for (const tc of choice.message.tool_calls) {
      ...
      try {
        ...
        // If arguments aren't valid JSON, pass as-is in a wrapper
...
function toOpenAITools(tools: Anthropic.Tool[]): OpenAITool[] {
  return tools.map((t) => {
    const fn: OpenAITool['function'] = {
      name: t.name,
      parameters: t.input_schema,
    };
    // description is optional on Anthropic.Tool; only set when present
...
export class OllamaProvider implements LlmProvider {
  ...
  private async doStream(
    ...
    // Accumulated state for reconstructing final message
    let accText = '';
    ...
    // Warn if tools were passed but stop_reason is end_turn with no tool calls
    if (params.tools && params.tools.length > 0 && accToolCalls.size === 0 && finishReason === 'stop') {
      process.stderr.write(
        '[koa] ollama: tools were passed but model returned end_turn — model may not support tool use\n',
      );
    }

    // Build the assembled OpenAI-format response for fromOpenAIResponse
  ...
  async create(params: LlmCallParams): Promise<Anthropic.Message> {
    ...
    return fromOpenAIResponse(data, params.model);
  }
}
